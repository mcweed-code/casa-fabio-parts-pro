...
export function ProductDetailPanel() {
  ...
  if (!productoSeleccionado) {
    return <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-4 text-center">
  ...
  return <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="h-[45%] bg-muted flex items-center justify-center p-2 shrink-0">
        <ImageLightbox src={imagenUrl} alt={productoSeleccionado.descripcion} className="w-full h-full object-contain max-h-full" fallback={<Package className="h-12 w-12 text-muted-foreground opacity-30" />} />
      </div>
      ...
      <div className="flex-1 flex flex-col p-2 overflow-hidden min-h-0">
        ...
        <div className="flex flex-wrap gap-1 mb-1">
          <span className="px-1.5 py-0.5 bg-accent/10 text-accent text-[10px] font-semibold rounded-full hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors" onClick={() => setTableCategory(productoSeleccionado.categoria)}>
            {productoSeleccionado.categoria}
          </span>
          <span className="px-1.5 py-0.5 bg-secondary text-secondary-foreground text-[10px] rounded-full hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors" onClick={() => setTableCategoryAndSubcategory(productoSeleccionado.categoria, productoSeleccionado.subcategoria)}>
        ...
        {/* Estado en pedido */}
        {itemEnPedido && <div className="p-1 bg-accent/10 rounded text-[10px] mt-1">
      ...
      <div className="shrink-0 p-2 border-t border-border bg-card/50">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <Input type="text" inputMode="numeric" value={cantidad} onChange={e => setCantidad(Math.max(1, parseInt(e.target.value) || 1))} className="text-center font-semibold h-6 w-10 text-xs px-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
            <Button variant="outline" size="icon" className="h-6 w-6" onClick={() => setCantidad(cantidad + 1)}>
          ...
          {mostrarCostos && <Select value={porcentaje.toString()} onValueChange={v => setPorcentaje(parseInt(v))}>
              ...
              <SelectContent>
                {PORCENTAJES_DISPONIBLES.map(p => <SelectItem key={p} value={p.toString()}>
                    {p}%
                  </SelectItem>)}
              </SelectContent>
            </Select>}
          ...
          {/* Botón eliminar */}
          {itemEnPedido && <Button onClick={handleEliminar} variant="outline" className="h-6 w-6 p-0">
